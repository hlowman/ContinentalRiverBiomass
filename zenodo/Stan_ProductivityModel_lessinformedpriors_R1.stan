...
data {
  ...
  vector [Ndays] tQ; // standardized discharge (to maximum discharge)
...
model {
  ...
  // Error priors
  sig_p ~ normal(0,2)T[0,]; // same as before
  sig_o ~ normal(mean(GPP_sd), sd(GPP_sd))T[0,]; // same as before
  
  // Param priors
  c ~ normal(0.5,0.25); // same as before
  s ~ normal(1.5,1); // same as before
  r ~ normal(0,1); // less informed
  lambda ~ normal(0,1); // less informed
