...
**We are interested in learning about streams' responses to disturbance (i.e., storm events). Specifically, by fitting a logistic population growth model to gross primary production estimates, we can gain information about growth parameters (e.g., maximum growth rate). We then aim to compare these population parameters describing algal biomass in streams across different sites and conditions to learn about these streams' resilience in the face of disturbance.**
...
From the initial 356 river sites in the Appling dataset, our filtered dataset consists of **207 sites** and **784 site-years** of data (Figures 4 and 5).
...
```{r results1, fig.align = "center", fig.cap = "Figure 7. Plot of mean maximum growth rates (r) and the log of mean carrying capacities (K) for individual sites. Note 14 sites with negative mean r and K values have been removed.", out.width = "50%", echo = FALSE}

knitr::include_graphics(here::here("figures", "teton_moresites", "r_K_allsites.jpg"))

```

```{r results2, fig.align = "center", fig.cap = "Figure 8. Boxplots of (A) mean maximum growth rates (r) and (B) mean carrying capacities (K) for individual sites grouped by stream order. Note, the same 14 sites that were removed from Figure 6 have been removed from these panels.", echo = FALSE}

knitr::include_graphics(here::here("figures", "teton_moresites", "r_k_strord.jpg"))

```

*Next Steps:* Following this meeting, we intend to re-fit the model with a random effect for site in order to pool information across sites.
