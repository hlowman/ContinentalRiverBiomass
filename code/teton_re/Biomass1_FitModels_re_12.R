...
# but with NEW, even TIGHTER PRIORS and predGPP moved into the body of the model.
...
init_Ricker <- function(...) {
  list(csite = rep(0.5,length.out = 10), 
       ...
       rsite = rep(0.2,length.out = 10), # NEW VALUES TO MATCH NEW PRIORS JUNE 2022
       lsite = rep(-0.03,length.out = 10), # HERE TOO, NEW AS OF JUNE 2022
       c = rep(0.3,length.out = 1), # NEW AS OF JULY 2022
       s = rep(1.0,length.out = 1), # NEW AS OF JULY 2022
       r = rep(0.2,length.out = 1), # ANOTHER NEW VALUE AS OF JUNE 2022
       lambda = rep(-0.03,length.out = 1), # NEW JUNE VALUE HERE AS WELL
...
PM_outputlist_Ricker_test <- stan("/project/modelscape/users/hlowman/jobscripts/teton_re/Stan_ProductivityModel2_Ricker_fixedinit_obserr_ts_wP_re_tightpriors_nopredGPP.stan",
                                  data = stan_data_l,
                                  chains = 3,
                                  iter = 5000,
                                  init = init_Ricker,
...
# export data
saveRDS(PM_outputlist_Ricker_test, "/project/modelscape/users/hlowman/jobresults/teton_re/stan_10rivers_output_Ricker_re_2022_07_07.rds")
