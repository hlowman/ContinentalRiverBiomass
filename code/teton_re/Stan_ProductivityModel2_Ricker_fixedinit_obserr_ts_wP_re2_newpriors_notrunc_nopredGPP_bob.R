...
transformed parameters {
  ...
  for (j in 1:(Ndays[h])) {
      ...
      P[j,h] = exp(-exp((ssite[h]*ssigma + s)*100*(tQ[j,h] - (csite[h]*csigma + c)))); // persistence as a function of discharge
...
model {
  ...
  for (j in 2:(Ndays[h])) {
    ...
    B[j,h] ~ normal(((B[(j-1),h] + (rsite[h]*rsigma + r) + (lsite[h]*lsigma + l)*exp(B[(j-1),h]))*P[j,h]), sig_p);
  ...
  csite[h] ~ normal(0,1); // previous form - normal(c,csigma)T[0,];
  ssite[h] ~ normal(0,1); // previous form - normal(s,ssigma)T[0,];
  rsite[h] ~ normal(0,1); // previous form - normal(r,rsigma);//T[0,]; // r can't be negative
  lsite[h] ~ normal(0,1); // previous form - normal(lambda,lsigma);//T[,0]; // lambda can't be positive
