...
model {
  ...
  csite[h] ~ normal(0,1)*csigma + c; // previous form - normal(c,csigma)T[0,];
  ssite[h] ~ normal(0,1)*ssigma + s; // previous form - normal(s,ssigma)T[0,];
  rsite[h] ~ normal(0,1)*rsigma + r; // previous form - normal(r,rsigma);//T[0,]; // r can't be negative
  lsite[h] ~ normal(0,1)*lsigma + l; // previous form - normal(lambda,lsigma);//T[,0]; // lambda can't be positive
