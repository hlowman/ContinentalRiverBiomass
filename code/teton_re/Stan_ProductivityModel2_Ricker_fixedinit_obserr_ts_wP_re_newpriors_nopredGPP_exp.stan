...
transformed parameters {
  ...
  for (j in 1:(Ndays[h])) {
      ...
      P[j,h] = exp(-exp(exp(ssite[h])*100*(tQ[j,h] - exp(csite[h])))); // persistence as a function of discharge
...
model {
  ...
  for (j in 2:(Ndays[h])) {
    ...
    B[j,h] ~ normal(((B[(j-1),h] + exp(rsite[h]) + exp(lsite[h])*exp(B[(j-1),h]))*P[j,h]), sig_p);
