...
model {
  ...
  rsite[h] ~ normal(r,rsigma);//T[0,]; // r can't be negative
  lsite[h] ~ normal(lambda,lsigma);//T[,0]; // lambda can't be positive
  ...
  // Param priors
  c ~ normal(0.3,0.15);//T[0,]; // revised JUL 2022
  s ~ normal(1.0,0.5);//T[0,]; // revised JUL 2022
  r ~ normal(0.2,0.1);//T[0,]; // r can't be negative
  lambda ~ normal(-0.03,0.01);//T[,0]; // lambda can't be positive
