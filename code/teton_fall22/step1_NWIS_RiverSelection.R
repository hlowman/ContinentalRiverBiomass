...
# some sites have multiple time resolutions (e.g., 15 and 30 minutes)

# Instead of filtering by K600 Rhat values for the whole site, 
# going to filter for daily records using the "NWIS" dataframe below

highq_site_names <- unique(highq_sites$site) ## 248 sites at the end

# Subset s based on high quality sites
...
# 341567 of original 490907 daily records remaining

# Confirm remaining number of sites
length(levels(as.factor(NWIS_sub$site_name))) ## 240 sites
...
sub_by_gap_sum <- sub_by_gap1 %>% group_by(site_name) %>% count() # 231 sites
...
# There are 198 unique sites in the TS_site dataset, with these new filters.

# I'm joining the full names to the TS dataset to better ID them when plotting.
name_bridge <- TS_site %>%
...
lapply(site_sub_list, function(x) ggsave(plot = plotting_covar(x),
       filename = paste("figures/teton_fall22/site_covariate_plots/",
