...
*H~1B~*: As **light availability** increases, the maximum growth rate will increase, since GPP tends to be highest in well-lit streams [(Bernhardt et al., 2022)](https://www.pnas.org/doi/10.1073/pnas.2121976119). Photoinhibition will likely not be a concern except in intermittent/ephemeral and low order streams.
...
*H~2B~*: As **land use** changes relative to undeveloped watersheds, the critical discharge threshold will change according to how the dominant land use in a watershed changes flow regimes. In predominantly agricultural watersheds, critical discharge thresholds will be greater, because flow will be less flashy and more predictable [(Poff et al., 2006)](https://www.sciencedirect.com/science/article/abs/pii/S0169555X06002546). In predominantly urban watersheds, critical discharge thresholds will be lower, because flow will be flashier and less predictable [(Blaszczak et al., 2019)](https://doi-org.unr.idm.oclc.org/10.1002/lno.11081).
...
```{r fig3, fig.align = "center", echo = FALSE, out.width = "75%", fig.cap = "Table 1. Summary statistics for all 182 stream sites included in the dataset."}
...
```{r pred1, fig.align = "left", fig.cap = "Figure 5. A comparison of gross primary productivity (GPP) estimates used during the initial model fit (dark green points) versus re-predicted GPP values using our model parameter estimates (light green line with 2.5% and 97.5% confidence intervals shaded). Note, this represents only one site (N.E. Anacostia River in Riverdale, MD) for one year (2011).", out.width = "100%", echo = FALSE}
